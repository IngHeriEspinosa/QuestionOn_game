...
export async function POST(
  ...
  try {
    ...
    const game = await getStore().toggleRevealOrNext(gameId);
    return NextResponse.json({ ok: true, status: game.status, revealed: game.revealed });
  } catch (error: unknown) {
    return toErrorResponse(error, "No se pudo avanzar");
  }
}
